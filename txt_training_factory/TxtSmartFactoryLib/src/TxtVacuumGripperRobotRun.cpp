...
void TxtVacuumGripperRobot::fsmStep()
{
	...
	{
		//printState(IDLE);
		is_din_value = dps.is_DIN();
		printf("is_din_value: %d\n",is_din_value);
		// creating_server_c();
		// printf("is_din_value: %d\n",is_din_value);
		//NFC requests
		if (reqNfcDelete)
...
{
	// std::cout<<"TxtVacuumGripperRobot::run() - calling moveAbs() 3 times after moveRef()\n";
	// /// Always need to execute the moveRef() for each axis.
	// axisX.moveRef();
	// axisX.moveAbs(17);
	// axisY.moveRef();
	// axisY.moveAbs(600);
	// axisZ.moveRef();
	// axisZ.moveAbs(16);
	// Now, all those functions will be called from a server
	// creating_server_c();
