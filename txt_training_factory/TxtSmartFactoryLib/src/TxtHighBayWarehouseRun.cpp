...
void TxtHighBayWarehouse::fsmStep()
{
	...
	case IDLE:
	{
		// When it receives the message from mqtt client then it triggers this FETCH_CONTAINER event
	...
	case FETCH_CONTAINER:
	{
		...
		// if(true)
		{
			std::cout<<"if (fetchContainer())\n";
			assert(mqttclient);
			mqttclient->publishHBW_Ack(HBW_FETCHED, reqVGRwp, TIMEOUT_MS_PUBLISH);
			std::cout<<"published the status. Now, triggering the STORE_WP\n";
