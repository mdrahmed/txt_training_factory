...
void TxtAxis1RefSwitch::moveRef()
{
	...
	resetCounter(); // no need
	std::cout<<"moveRef chM: "<<static_cast<int>(chM)<<"\n";
	if (chM < 8)
	{
		assert(pT->pTArea);
	...
	if ((status != AXIS_READY)&&(status != AXIS_NOREF)) {
		// std::cout<<"moveRef AXIS_NOREFAXIS_READY & !AXIS_NOREF\n";
	...
	while (true)
	{
		...
		{
			// std::cout<<"moveRef isSwitchPressed\n";
			setMotorOff();
			break;
		}
		//check stop req
		if (stopReq) {
			// std::cout<<"moveRef stopReq\n";
			setMotorOff();
		...
		if (diff_s > TIMEOUT_S_MOVEREF) {
			// std::cout<<"moveRef diff_s > TIMEOUT_S_MOVEREF\n";
	...
	// std::cout<<"moveRef loop ran for: "<<i<<" times\n";
	if (status == AXIS_MOVING_REF)
	{
		// std::cout<<"moveRef status == AXIS_MOVING_REF\n";
		resetCounter();
		pos = 0;
...
bool TxtAxis1RefSwitch::moveAbs(uint16_t p) {
	std::cout<<"moveAbs(uint16_t p)- p: "<<p<<"\n";
...
void TxtAxis1RefSwitch::moveRight(uint16_t steps, uint16_t* p) {
	assert(p!=NULL);
	std::cout<<"moveRight(uint16_t steps, uint16_t* p)- steps: "<<steps<<" p: "<<*p<<"\n";
