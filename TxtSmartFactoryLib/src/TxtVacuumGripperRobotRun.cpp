...
auto vgr_to_hbw_ms = std::chrono::high_resolution_clock::now();
auto vgr_to_mpo_ms = std::chrono::high_resolution_clock::now();
auto end_ms = std::chrono::high_resolution_clock::now();

namespace ft
{
	time_t start, vgr_to_hbw = 0, vgr_to_mpo = 0, end;
	...
	void TxtVacuumGripperRobot::fsmStep()
	{
		...
		std::cout << "TxtVacuumGripperRobot" << std::endl;
		...
		std::cout << "reqOrder:" << reqOrder << std::endl;
		std::cout << "reqNfcRead:" << reqNfcRead << std::endl;
		std::cout << "reqNfcDelete:" << reqNfcDelete << std::endl;
		std::cout << "reqJoyData:" << reqJoyData << std::endl;
		std::cout << "reqMPOstarted:" << reqMPOstarted << std::endl;
		std::cout << "reqHBWstored:" << reqHBWstored << std::endl;
		std::cout << "reqHBWfetched:" << reqHBWfetched << std::endl;
		std::cout << "reqHBWcalib_nav:" << reqHBWcalib_nav << std::endl;
		std::cout << "reqHBWcalib_end:" << reqHBWcalib_end << std::endl;
		std::cout << "reqSLDcalib_end:" << reqSLDcalib_end << std::endl;
		std::cout << "reqSLDsorted:" << reqSLDsorted << std::endl;
		...
		*/
		case IDLE:
		{
			...
			// Just type anything except the original command
			// Commands are like - order blue, order red, order white, ******command can be send only via c32******
			// The TxtFactoryVGR will be listening ******to send command start ./c32 and type any one of the above orders******
			creating_server_c();
			...
			{
				time(&vgr_to_hbw);
				std::cout << "vgr_to_hbw start time: " << vgr_to_hbw;
				vgr_to_hbw_ms = std::chrono::high_resolution_clock::now();
		...
		{
			time(&vgr_to_mpo);
			std::cout << "vgr_to_mpo start time: " << vgr_to_mpo;
			vgr_to_mpo_ms = std::chrono::high_resolution_clock::now();
	...
	void TxtVacuumGripperRobot::run()
	{
		...
		while (!m_stoprequested)
		{
			...
			std::cout << "VGR: TOTAL Time taken by program is : " << std::fixed
			...
			if (vgr_to_hbw == 0)
			{
				std::cout << "VGR to HBW time not taken" << std::endl;
			}
			else
			{
				time_taken = double(end - vgr_to_hbw);
				std::cout << "VGR to HBW time taken: " << std::fixed
						  << time_taken << std::setprecision(5);
				std::cout << " sec " << std::endl;

				duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_ms - vgr_to_hbw_ms);
				std::cout << "vgr2hbw miliseconds time taken: " << duration.count() << "ms" << std::endl;
			}
			if (vgr_to_mpo == 0)
			{
				std::cout << "VGR to MPO time not taken" << std::endl;
			}
			else
			{
				time_taken = double(end - vgr_to_mpo);
				std::cout << "VGR to MPO time taken: " << std::fixed
						  << time_taken << std::setprecision(5);
				std::cout << " sec " << std::endl;

				duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_ms - vgr_to_mpo_ms);
				std::cout << "vgr2mpo miliseconds time taken: " << duration.count() << "ms" << std::endl;
			}
		}
